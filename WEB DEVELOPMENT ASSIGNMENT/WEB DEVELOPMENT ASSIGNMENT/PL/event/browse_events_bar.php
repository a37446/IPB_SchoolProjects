...
for ($i=0;$i<count($event_array);$i++) {
            ...
            $output = $output . '<h2 class="event_title">' . $event_array[$i]->getMuseum_name() .'<br>' . $event_array[$i]->get_Event_name() . "</h2>";
            $output = $output . '<img class="event_icon" src="'. $event_array[$i]->getEvent_image() .' alt="Image not available" ">';
