...
class DAL_Work{ 
    ...
    static function create_work($name,$material,$type,$date,$description,$Room_name,$Work_img_name,$Work_path) {
        ...
        $db_conn=new DB();
        $conn=$db_conn->connect();
        $stmt = $conn->prepare("SELECT * FROM work INNER JOIN artist_work ON artist_work.Work_idWork=work.idWork INNER JOIN artist on artist.idArtist=artist_work.Artist_idArtist INNER JOIN room ON work.Room_idRoom=room.idRoom ORDER BY idRoom");
        $stmt->execute();        
        $stmt->setFetchMode(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, 'Work'); 
        $works_array = [];
        $index=0;
        ...
        $db_conn=new DB();
        $conn=$db_conn->connect();
        $stmt = $conn->prepare("SELECT * FROM work INNER JOIN artist_work ON artist_work.Work_idWork=work.idWork INNER JOIN artist on artist.idArtist=artist_work.Artist_idArtist INNER JOIN room ON work.Room_idRoom=room.idRoom ORDER BY Artist_name");
        $stmt->execute();        
        $stmt->setFetchMode(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, 'Work'); 
        $works_array = [];
        $index=0;
